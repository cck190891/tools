from linovelib_book_mult import linovelib_loader 
...
def get_img(path,pic_len,all_img_url,cover_url):
    ...
    for chapter in all_img_url:
        fail=[]
        pool = ThreadPool(16)
        results1 = pool.map(get_one,all_img_url[chapter])
        pool.close()
        pool.join()
        
        while fail:
            pool = ThreadPool(16)
            results1 = pool.map(get_one,fail)
            pool.close()
            pool.join()
...
def make_mimetype(out_path):
    with open(os.path.join(out_path,'mimetype'),'w') as f:
...
def make_epub(path,book_title):
    ...
    for dir in os.listdir(path):
        ...
        book_file_name=book_title+' '+dir+'.epub'
        with zipfile.ZipFile(os.path.join(path,dir,book_file_name), mode='w') as zf:
            zf.write(os.path.join('mimetype'))
        with zipfile.ZipFile(os.path.join(path,dir,book_file_name), mode='a') as zf:
            for r,_,f in os.walk(os.path.join('OEBPS')):
                for file in f:
                    zf.write(os.path.join(r,file))
            for r,_,f in os.walk(os.path.join('META-INF')):
                for file in f:
                    zf.write(os.path.join(r,file))
        os.rename(os.path.join(path,dir,book_file_name), os.path.join(path,book_file_name))
